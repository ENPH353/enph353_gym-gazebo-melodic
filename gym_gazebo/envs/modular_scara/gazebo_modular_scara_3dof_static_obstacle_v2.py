...
class GazeboModularScara3DOFStaticObstaclev2Env(gazebo_env.GazeboEnv):
    """
    ...
    def setTorqueThreshold(self, th):
        self.torque_th=th
    ...
    def _step(self, action):
        ...
        #  TORQUE BASED REWARD SHAPING
            ...
            #elif torque_value > 0.01 and torque_value > 0.1 and torque_value < 1:
                    #self.reward = self.reward - (abs(torque_value)) 
                    #print("\n Reward, torque penalization", self.reward)
            #elif torque_value > 0.01 and torque_value > 1:
                    #self.reward = self.reward - (abs(torque_value)) * 10
                    #print("\n Reward, torque penalization", self.reward)
                # here we want to fetch the positions of the end-effector which are nr_dof:nr_dof+3
        if(self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)])<0.005):
            self.reward = 1 - self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)]) # Make the reward increase as the distance decreases
            print("Reward is: ", self.reward)
        #elif(self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)])<0.01):
            ...
            #print("Reward is: ", self.reward)
        #elif(self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)])<0.05):
            #self.reward = 0.3 - self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)]) # Make the reward increase as the distance decreases
            #print("Reward is: ", self.reward)
        #elif(self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)])<0.08):
            #self.reward = 0.05 - self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)]) # Make the reward increase as the distance decreases
            #print("Reward is: ", self.reward)
        elif(self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)])>0.25):
             self.reward = - self.rmse_func(self.ob[self.scara_chain.getNrOfJoints():(self.scara_chain.getNrOfJoints()+3)]) *20 # Make the reward increase as the distance decreases
             print("Reward is: ", self.reward)
        else:
            self.reward = self.reward_dist

        print("\n RMSE DISTANCE: ", self.reward_dist)
        # Calculate if the env has been solved
        done = (bool(abs(self.reward_dist) < 0.005)) or (self.iterator > self.max_episode_steps)
        ...
        if self._torque_msg is not None:
            ...
            torque_value = np.sqrt(torque_x * torque_x + torque_y * torque_y + torque_z * torque_z) / 1000
            print("\n Torque value:", torque_value)
            if torque_value > 0.001 and torque_value < 0.1:
                    self.reward = self.reward - (abs(torque_value)) * 5
                    #self.reward = self.reward - (abs(torque_value)) * 200
                    print("\n Reward, torque penalization", self.reward)
            elif torque_value > 0.001 and torque_value > 0.1 and torque_value < 1:
                    self.reward = self.reward - (abs(torque_value)) * 10
                    #self.reward = self.reward - (abs(torque_value)) * 1000
                    print("\n Reward, torque penalization", self.reward)
            elif torque_value > 0.001 and torque_value > 0.1 and torque_value > 1:
                    self.reward = self.reward - (abs(torque_value)) * 100
                    #self.reward = self.reward - (abs(torque_value)) * 10000
                    print("\n Reward, torque penalization", self.reward)
            elif torque_value < 0.001:
