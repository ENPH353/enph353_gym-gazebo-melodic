...
def stateStrToState(state_str):
    ...
    if len(new_period_indices) == 3:
        ...
        retVal.append(float(new_str[new_period_indices[2]-1:]))
    else:
        print("Not 3 periods")
        return None
...
if __name__ == '__main__':
    ...
    for state_action, reward in data.items():
        ...
        if not state_action_reward is None:
            print("Parsed! State {} {} {}. Action {}. Reward {}".format(state_action_reward.x, state_action_reward.y, state_action_reward.Y, state_action_reward.action, state_action_reward.reward))
            print("***")
